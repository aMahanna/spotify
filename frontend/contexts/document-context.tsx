...
export function DocumentProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isInitialized) {
      try {
        const savedDocuments = localStorage.getItem('SpotifyKG_documents')
        if (savedDocuments) {
    ...
    if (isInitialized) {
      try {
        if (documents.length > 0) {
          ...
          localStorage.setItem('SpotifyKG_documents', JSON.stringify(documentsToSave));
          console.log(`Saved ${documents.length} documents to localStorage`);
        } else {
          // Clear localStorage if documents array is empty
          localStorage.removeItem('SpotifyKG_documents');
